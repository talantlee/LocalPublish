...
//Upload
//AsyncCallback callback = new AsyncCallback(FinishedOperation);
//Singleton.InstanceFtpClient.BeginUploadDirectory(@"C:\test2", true, callback);

//Download
//AsyncCallback callback = new AsyncCallback(FinishedOperation);
//Singleton.InstanceFtpClient.BeginDownloadDirectory("testmm", @"C:\HaHa", callback);

//private void FinishedOperation(IAsyncResult result)
//{
...
namespace LocalPublish
{
    ...
    public class FtpClient
    {
        ...
        System.Windows.Forms.ProgressBar _progressbar = null;
        System.Windows.Forms.Label _lblinfo = null;
        /// <summary>
        /// Default contructor
        /// </summary>
        public FtpClient()
        ...
        public FtpClient(string server, string username, string password, int timeoutSeconds, int port):this(server,username,password,timeoutSeconds,port,null,null)
        {
            
        }

        public FtpClient(string server, string username, string password, int timeoutSeconds, int port, System.Windows.Forms.ProgressBar progressbar, System.Windows.Forms.Label lblinfo)
        {
            this.server = server;
            this.username = username;
            this.password = password;
       ...
       //     Debug.WriteLine("Opening connection to " + this.server, "FtpClient");
            ...
            try
            {
                ...
                //  IPHostEntry hostE= Dns.GetHostEntry(this.server).HostName;
                //  string hostname = Dns.GetHostEntry(this.server).HostName;
                addr = Dns.GetHostAddresses(this.server)[0];
        ...
        public string[] GetDirectoryList(string mask)
        {
        ...
        /// <param name="remFileName"></param>
        /// <param name="locFileName"></param>
        /// <param name="resume">true:断点续传</param>
        public void Download(string remFileName, string locFileName, Boolean resume)
        {
            ...
            //
            Socket cSocket = createDataSocket();
            long fileSize=this.GetFileSize(remFileName);
            long offset = 0;
            if (_progressbar != null)
            {
                if (resume && output.Length>1)
                {
                    if (output.Length < fileSize)
                        _progressbar.Value = Convert.ToInt32((double)output.Length / fileSize * 100);
                    else
                    {
                        output.SetLength(0);
                ...
                _progressbar.Value = 2;
                if (_lblinfo != null)
                    _lblinfo.Text = "正在下载 " + remFileName;// +" from " + server + "/" + remotePath;
                _progressbar.Step = Convert.ToInt32((double)fileSize / (BUFFER_SIZE * 100));
            ...
            if (resume)
            {
                ...
                if (offset > 0)
                {
                    ...
                    if (this.resultCode != 350)
                    {
                        ...
                        offset = 0;
                        if (_lblinfo != null)
                            _lblinfo.Text = "不支持断点续传:" + result.Substring(4);
                    }
                    else
                    {
                        //if (_lblinfo != null)
                        //    _lblinfo.Text = "正在验证" + remFileName+"";
        ...
        /// <summary>
        /// DownloadDirectory a directory and its file contents
        /// </summary>
        /// <param name="path">FTP服务器上的一个目录</param>
        /// <param name="localpath">下载到本地的目录</param>
        public void DownloadDirectory(string path, string localpath)
        {
            string[] dirs = path.Replace("/", @"\").Split('\\');
            string rootDir = dirs[dirs.Length - 1];
            string currentpath = localpath + "\\" + rootDir;
            
            if (this.GetFileList(path).Length < 1)
            {
            ...
            string[] con_str = GetFileList();
            if (con_str.Length > 0)
            {
                int tempcount = con_str.Length - 1;//去掉最后一个""
                for (int i = 0; i < tempcount; i++)
                {
                    string file = con_str[i];
                    if (file.Length != 0)
                    {
                      //  if (file.IndexOf('.') > 0)
                       // {
                            string currentlocalfile = currentpath + "\\" + file;
                                this.Download(file, currentlocalfile,true);
                          
                      //  }
                        //else
                        //{
                        //    this.DownloadDirectory(con_str[i], currentpath + "\\" + con_str[i]);
                        //}
                    }
                }
            }
        ...
        public int SubRootCount = 0;
        public void PureUploadSub(string fullpath,string relativepath)
        {
            if (relativepath.IndexOf("/") > -1) relativepath = relativepath.Replace("/", @"\");
            string[] dirs = relativepath.Split('\\');
            if (dirs.Length == 1 || relativepath=="")
            {
                Upload(fullpath, false);
                return;
            }
            SubRootCount += 1;
            string rootDir = dirs[0];
            ...
            PureUploadSub(fullpath, subPath);
        ...
        private Socket createDataSocket()
        {
            ...
            try
            {
                ...
                ep = new IPEndPoint(Dns.GetHostAddresses(ipAddress)[0], port);
                socket.Connect(ep);
            }
            catch (Exception ex)
            {
...
*/
        ...
        private delegate void DownloadDirectoryCallback(string path, string localpath);
        public System.IAsyncResult BeginDownloadDirectory(String path, string localpath, System.AsyncCallback callback)
        {
            DownloadDirectoryCallback ftpCallback = new DownloadDirectoryCallback(this.DownloadDirectory);
            return ftpCallback.BeginInvoke(path, localpath, callback, null);
        }
